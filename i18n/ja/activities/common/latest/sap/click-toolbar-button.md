...
sidebar_position: 3
description: "Click Toolbar Button アクティビティに関するドキュメント。"
...
システムまたはアプリケーションの SAP ツールバーからボタンをクリックします。画面上で SAP ツールバーを指定すると、利用可能なすべてのボタンを含むリストがアクティビティ内に表示されます。
...
* **Button (文字列)*** - SAP ツールバー上でクリックするボタンの名前（例: `"Save"`）。画面上でツールバーを選択した後、アクティビティ内のドロップダウン メニューからボタンを選択できます。
...
* **Selector (文字列)*** - ターゲットとなる SAP ツールバーを特定するための XML セレクター フラグメント。画面上でツールバーを選択すると、自動的に生成されます。
