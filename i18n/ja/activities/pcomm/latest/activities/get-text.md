...
sidebar_position: 2
description: "Get Text アクティビティに関するドキュメント。"
...
指定された Row（行）、Column（列）、および Length（文字数）に基づいて、PCOMM 画面からテキスト文字列を抽出します。
...
* **Row (Int32)** - テキスト抽出を開始するエミュレーター画面上の行座標。
* **Column (Int32)** - テキスト抽出を開始するエミュレーター画面上の列座標。
* **Length (Int32)** - 指定された Row と Column の座標から抽出する文字数。
